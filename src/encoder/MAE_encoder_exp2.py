...
class MAE_encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks_decoder = nn.ModuleList([
            Block(decoder_embed_dim, num_heads, dim_head, mlp_ratio, qkv_bias=True)
            for _ in range(4)])
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        #for blk in self.blocks_decoder:
            #x = blk(x)
        x = x.mean(dim=1) 
...
class MAE(nn.Module):
    ...
    def pretrain(self, dataset, dataset_denoise, batch_size, lr, pretrain=True):
        if pretrain:
            pretrain_mae(self.mae, dataset, dataset_denoise, batch_size, lr, self.pretrain_path)
...
def pretrain_mae(model, dataset, dataset_denoise, batch_size, lr, pretrain_path):
    cuda = torch.cuda.is_available()
    device = torch.device("cuda:1" if cuda else "cpu")
    batch_size = batch_size
    data = torch.Tensor(dataset).to(device)
    x = data.unsqueeze(1)
    data_denoise = torch.Tensor(dataset_denoise).to(device)
    x_denoise = data_denoise.unsqueeze(1)
    ...
    for epoch in tqdm(range(40), desc='Pretrain'):
        total_loss = 0.
        ...
        for batch in range(n_batch):
            ...
            #x_denoise_train = x_denoise[idx, :, :, :].to(device)
            
            optimizer.zero_grad()
            
            #z1, loss1, _, q1 = model(x_denoise_train)
            z2, loss2, _, q2 = model(x_train)
            
            #caculate_clrloss = ContrastiveLoss(batch_size = len(x_train))
            #contras_loss = 0.01*caculate_clrloss(z1, z2)
            contro_loss = loss2
            loss = contro_loss
